...
# ImТипыmutable - Неизенияемые int, float, str, tuple, bool, frozenset, None
...
#               ... -2 -1
# string = "some string" string[index]
...
# словарь - изменяемый, неупорядоченный тип данных, в котором хранятся пары в виде ключ - значение. Ключ в словаре - любой неизменяемый тип данных. Каждый ключ - уникальный Значение в словаре - любой тип данных. Ключ от значения отделяется двоеточием. Пары отделяются запятыми.
# Синтаксиc:
...
# цикл - блок кода, который будет выполнятся несколько раз. Каждый круг цикла называется итерацией.
...
# for - цикл, который производит итерации над последовательностью (list, dict, set, str, tuple) в цикле for мы можем выполнять различные операции над каждым элементом последовательности
# Синтаксиc: for элемент in последовательность: какие-то действия

# While

# while - цикл, который производит какие-то действия пока условие будет True
# Синтаксиc: while условие: какие-то действия

# Conditions

# условия - оператор, который позволяет нам выполнять или не выполнять какой-то кусочек кода, который находится в теле условия.
# Синтаксиc: if условие: тело

# if True: print("Условие верное")

# Также мы можем создавать конструкции из условных операторов.
# if условие: тело1 elif условие: тело2 else: тело3
...
# Синтаксис: действие for элемент in последовательность [if условие]
...
# функция - именованный блок кода, выполняющий какие-то действия и возвращающий какой-то результат. Мы можем вызывать функцию, обращаясь к ней по имени и используя круглые скобочки. код, который написан внутри функции будет работать только при вызове функции функции могут принимать данные
...
# map - функция, которая выполняет действия над каждым элементом последовательности и возвращает генератор новой последовательности она принимает 2 аргумента:
# функция, которая принимает 1 аргумент
# последовательность
# Синтаксиc:
...
# filter - функция, которая возвращает последовательность из элементов, соответствующих условию она принимает 2 аргумента:
# функция, которая принимает 1 аргумент, которая возвращает булевое значение (если True, то добавляется)
# последовательность
# Синтаксиc:
